...
class Model:
    ...
    def is_neighbor(self,x,y,j,k):
        if (x, y, j, k) in self.distance_cache:
            return self.distance_cache[(x, y, j, k)]
        dx = abs(x - j)
        dy = abs(y - k)
        ...
        # 将计算结果存入缓存
        self.distance_cache[(x, y, j, k)] = result
        self.distance_cache[(j, k, x, y)] = result  # 距离是对称的，因此也需要存储 (j, k, x, y) 的结果
        return result
